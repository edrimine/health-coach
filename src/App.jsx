...
// ── Storage layer (localStorage with JSON wrapper) ─────────────────────────
const LS = {
...
const ARCHIVE_KEY = "hc_week_2026_05_04";
const ARCHIVE_DATA = { cardio: 2, strength: 0, steps: 3, notes: "Two runs (May 7 + May 9). Missed strength. 3 of 4 step days.", savedAt: "2026-05-10T14:32:00Z" };
const CURRENT_WEEK_KEY = "hc_week_2026_05_11";
const SEEDED_CURRENT = { cardio: 0, strength: 0, steps: 0, notes: "" };
const SEEDED_WEIGHTS = [
  { key: "hc_weight_2026_05_07", weight: 213.8 },
  { key: "hc_weight_2026_05_10", weight: 215.3 },
];
...
function getMonday(d = new Date()) {
  ...
  mon.setDate(d.getDate() + diff); return mon;
}
function weekKey(weeksAgo = 0) {
  const mon = getMonday(); mon.setDate(mon.getDate() - weeksAgo * 7);
  return `hc_week_${mon.getFullYear()}_${String(mon.getMonth()+1).padStart(2,"00")}_${String(mon.getDate()).padStart(2,"00")}`;
}
function weekLabel(key) {
  const p = key.replace("hc_week_","").split("_");
  if (p.length < 3) return key;
  return new Date(+p[0],+p[1]-1,+p[2]).toLocaleDateString("en-US",{month:"short",day:"numeric"}) + " week";
}
function todayKey() {
  const d = new Date();
  return `hc_weight_${d.getFullYear()}_${String(d.getMonth()+1).padStart(2,"00")}_${String(d.getDate()).padStart(2,"00")}`;
}
function dateLabel(key) {
  const p = key.replace("hc_weight_","").split("_");
  if (p.length < 3) return key;
...
function WeightChart({ entries }) {
  ...
  const sorted = [...entries].sort((a,b)=>a.key.localeCompare(b.key)).slice(-30);
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%", overflow:"visible" }}>
      ...
      {sorted.map((e,i)=><g key={e.key}><circle cx={xp(i)} cy={yp(e.weight)} r={3} fill={ACCENT}/>{(i===0||i===sorted.length-1||i%7===0)&&<text x={xp(i)} y={H-4} textAnchor="middle" fill={MUTED} fontSize={8}>{dateLabel(e.key)}</text>}</g>)}
    </svg>
  );
}
...
function WorkoutsTab() {
  ...
  const [lastDone, setLastDone] = useState(() => LS.get("hc_lastWorkout") || {});
  ...
  function markDone(id) {
    ...
    setLastDone(updated); LS.set("hc_lastWorkout", updated);
  }
...
export default function HealthCoach() {
  ...
  const [currentWeek, setCurrentWeek] = useState(() => LS.get(CURRENT_WEEK_KEY) || { ...SEEDED_CURRENT });
  ...
  useEffect(() => {
    ...
    if (!LS.get(ARCHIVE_KEY)) LS.set(ARCHIVE_KEY, ARCHIVE_DATA);

    // Load week history — collect all hc_week_ keys + known seeds
    const keysToCheck = new Set([ARCHIVE_KEY]);
    for (let i=1; i<=16; i++) keysToCheck.add(weekKey(i));
    LS.keys("hc_week_").forEach(k => keysToCheck.add(k));

    const hist = [];
    for (const k of keysToCheck) {
      if (k === CURRENT_WEEK_KEY) continue;
      const data = LS.get(k);
      if (data) hist.push({ key: k, data });
    }
    hist.sort((a,b) => b.key.localeCompare(a.key));
    setWeekHistory(hist);

    // Load weight entries — collect all hc_weight_ keys + seeds
    const wKeysToCheck = new Set();
    LS.keys("hc_weight_").forEach(k => wKeysToCheck.add(k));
    SEEDED_WEIGHTS.forEach(sw => wKeysToCheck.add(sw.key));

    const wEntries = [];
    for (const k of wKeysToCheck) {
      const raw = localStorage.getItem(k);
      if (raw) {
        const val = parseFloat(raw);
        if (!isNaN(val)) wEntries.push({ key: k, weight: val });
      }
    }
    // Seed missing weights
    for (const sw of SEEDED_WEIGHTS) {
      if (!wEntries.find(e=>e.key===sw.key)) {
        localStorage.setItem(sw.key, String(sw.weight));
        wEntries.push({ key: sw.key, weight: sw.weight });
      }
    }
    setWeightEntries(wEntries.sort((a,b)=>a.key.localeCompare(b.key)));
  }, []);

  // ── Persist current week on change ──
  useEffect(() => { LS.set(CURRENT_WEEK_KEY, currentWeek); }, [currentWeek]);
  ...
  function archiveWeek() {
    const saved = { ...currentWeek, savedAt: new Date().toISOString() };
    LS.set(CURRENT_WEEK_KEY, saved);
    setWeekHistory(h => [{ key:CURRENT_WEEK_KEY, data:saved }, ...h.filter(x=>x.key!==CURRENT_WEEK_KEY)]);
  ...
  function logWeight() {
    ...
    setWeightEntries(prev => [...prev.filter(e=>e.key!==k), {key:k,weight:val}].sort((a,b)=>a.key.localeCompare(b.key)));
    setWeightInput("");
    flash("✓ Weight logged");
  }

  const cP=currentWeek.cardio/WEEK_TARGETS.cardio, sP=currentWeek.strength/WEEK_TARGETS.strength, stP=currentWeek.steps/WEEK_TARGETS.steps;
  ...
  const todayLogged=weightEntries.some(e=>e.key===todayKey());
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG, color:TEXT, fontFamily:"'DM Sans','IBM Plex Sans',system-ui,sans-serif", paddingBottom:60 }}>
      ...
      <div style={{ maxWidth:700, margin:"0 auto", padding:"24px 20px" }}>
        ...
        {tab==="tracker"&&<>
          <div style={{ background:"#1a2e1a", border:`1px solid #2d5a2d`, borderRadius:10, padding:"10px 14px", marginBottom:20, fontSize:13, color:"#86efac" }}>
            <strong>Synced May 10 · Week of May 4 archived ✓</strong> — VO₂ Max 35.8 ↑ · Weight 215.3 lbs
          </div>
          <SectionHead>THIS WEEK (MAY 11+)</SectionHead>
          ...
          <Card>
            ...
            <div style={{ fontSize:13, color:MUTED, lineHeight:1.75 }}>Tell Claude <span style={{ color:TEXT, fontStyle:"italic" }}>"update my activity"</span> any time. On Sundays, the previous week is auto-archived.</div>
          </Card>
        </>}
        ...
        {tab==="weight"&&<>
          ...
          <Card>{weightEntries.length===0?<div style={{ color:MUTED, fontSize:13 }}>No weight logged yet.</div>:[...weightEntries].reverse().slice(0,14).map(e=>(
            <div key={e.key} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:`1px solid ${BORDER}` }}>
              <span style={{ color:MUTED, fontSize:13 }}>{dateLabel(e.key)}</span>
        ...
        {tab==="history"&&<>
          ...
          {weekHistory.length===0
            ...
            :weekHistory.map(w=>{
              const d=w.data, score=Math.round((Math.min(d.cardio,WEEK_TARGETS.cardio)/WEEK_TARGETS.cardio+Math.min(d.strength,WEEK_TARGETS.strength)/WEEK_TARGETS.strength+Math.min(d.steps,WEEK_TARGETS.steps)/WEEK_TARGETS.steps)/3*100), open=histOpen===w.key;
              return(
                <Card key={w.key} onClick={()=>setHistOpen(open?null:w.key)} highlight={open} style={{ marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      <div style={{ fontWeight:700, fontSize:14 }}>{weekLabel(w.key)}</div>
                      <div style={{ fontSize:12, color:MUTED, marginTop:3 }}>🏃 {d.cardio}/{WEEK_TARGETS.cardio} &nbsp;·&nbsp; 💪 {d.strength}/{WEEK_TARGETS.strength} &nbsp;·&nbsp; 🚶 {d.steps}/{WEEK_TARGETS.steps}</div>
                    </div>
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  {open&&<div style={{ marginTop:12, paddingTop:12, borderTop:`1px solid ${BORDER}`, fontSize:13, color:"#94a3b8", lineHeight:1.7 }}>{d.notes||<span style={{ color:MUTED }}>No notes for this week.</span>}</div>}
                </Card>
              );
            })
          }
